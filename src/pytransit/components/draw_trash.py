...
script_directory = os.path.dirname(os.path.realpath(__file__)) # should be src/pytransit/components/draw_trash.py
font = ImageFont.truetype("%s/DejaVuSans.ttf" % script_directory,size=12) # should be in same directory as draw_trash.py
...
def draw_reads(
    ...
    for i, TA in enumerate(ta_sites):
        ...
        read_h = normalize(
            NORM_READS[i], 0, max_read, new_min_h, new_max_h
        )  # height of read line
...
def draw_ta_sites(
    draw, ta_sites, start_x=0, start_y=0, width=200, height=0, start=0, end=500, lwd=2
):
    ...
    for i, TA in enumerate(ta_sites):
        TApos = normalize(TA, start, end, new_min_w, new_max_w)
        draw.line(
            [(TApos, start_y + 0), (TApos, start_y + height)], width=lwd, fill="black"
        )
...
def draw_features(draw, GENES, orf2data, start, end, start_x, start_y, width, height):
    ...
    for gene in GENES:
        ...
        if strand == "-":
            if gene_start >= start:
                draw.rectangle(
                    ((norm_start, start_y + 5), (norm_end, start_y + gene_h - 5)),
                    fill=color,
                )
        ...
        if not name.startswith("non-coding"):
            ...
            if abs(norm_start - norm_end) >= name_text_w:
                draw.text(
                    (
                        norm_start + (abs(norm_start - norm_end) - name_text_w) / 2.0,
                        start_y + gene_h + text_h,
                    ),
                    name,
                    font=font,
                    fill="black",
...
def draw_genes(
    draw, GENES, orf2data, start, end, start_x, start_y, width, height, doTriangle=True
):
    ...
    for gene in GENES:
        ...
        if strand == "-":
            ...
            if gene_start >= start:
                draw.rectangle(
                    (
                        (norm_start + triangle_size, start_y + 5),
                        (norm_end, start_y + gene_h - 5),
                    ),
                    fill="blue",
                )
                if doTriangle:
                    draw.polygon(
                        [
                            (norm_start + triangle_size, start_y),
                            (norm_start + triangle_size, start_y + gene_h),
                            (norm_start, start_y + gene_h / 2.0),
                        ],
                        fill="blue",
                    )
            ...
            if gene_end <= end:
                draw.rectangle(
                    (
                        (norm_start, start_y + 5),
                        (norm_end - triangle_size, start_y + gene_h - 5),
                    ),
                    fill="blue",
                )
                if doTriangle:
                    draw.polygon(
                        [
                            (norm_end - triangle_size, start_y),
                            (norm_end - triangle_size, start_y + gene_h),
                            (norm_end, start_y + gene_h / 2.0),
                        ],
                        fill="blue",
                    )
            else:
...
def draw_canvas(
        ...
        for i, read in enumerate(data):
            ...
            if start <= pos <= end:
                ...
                if (
                    gene == "non-coding"
                    and len(GENES) > 0
                    and not GENES[-1].startswith("non-coding")
                ):
                    gene += "_%d" % nc_count
                    nc_count += 1
                if j == 0:
                    ...
                    for f, f_hash in enumerate(feature_hashes):
                        ...
                        if feat not in FEATURES[f]:
                            FEATURES[f].append(feat)
                temp.append(read)
        READS.append(temp)
    ...
    for j in range(len(fulldata)):
        ...
        draw_scale(
            draw, start_x + read_w + padding_w + 2, start_y - 100 + 10, 70, max_reads[j]
        )
