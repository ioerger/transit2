...
class QualityControlFrame(wx_frame):
    def __init__(self, parent, wig_ids):
        ...
        try:
            ...
            self.plotsScrolledWindow = wx.ScrolledWindow(
            ...
            self.plotsBitmap1 = wx.StaticBitmap(
                self.plotsScrolledWindow,
                wx.ID_ANY,
                wx.NullBitmap,
                wx.DefaultPosition,
                wx.DefaultSize,
            ...
            self.plotsBitmap2 = wx.StaticBitmap(
                self.plotsScrolledWindow,
                wx.ID_ANY,
                wx.NullBitmap,
                wx.DefaultPosition,
            ...
            self.plotsBitmap3 = wx.StaticBitmap(
                self.plotsScrolledWindow,
                wx.ID_ANY,
                wx.NullBitmap,
                wx.DefaultPosition,
            ...
            self.statsScrolledWindow = wx.ScrolledWindow(
                self.m_scrolledWindow1,
                wx.ID_ANY,
                wx.DefaultPosition,
                wx.DefaultSize,
 ...
 This may take a long time depending on the normalization method chosen."""
            ...
            self.statsListCtrl = wx.ListCtrl(
                self.statsScrolledWindow,
                wx.ID_ANY,
                wx.DefaultPosition,
                wx.Size(-1, 140),
            ...
            self.statsListCtrl.InsertColumn(
                1, "Density", wx.LIST_FORMAT_CENTRE, width=85
            )
            self.statsListCtrl.InsertColumn(
                2, "Mean Read", wx.LIST_FORMAT_CENTRE, width=85
            )
            self.statsListCtrl.InsertColumn(
                3, "NZMean Read", wx.LIST_FORMAT_CENTRE, width=115
            )
            self.statsListCtrl.InsertColumn(
                4, "NZMedian Read", wx.LIST_FORMAT_CENTRE, width=125
            )
            self.statsListCtrl.InsertColumn(
                5, "Max Read", wx.LIST_FORMAT_CENTRE, width=85
            )
            self.statsListCtrl.InsertColumn(
                6, "Total Reads", wx.LIST_FORMAT_CENTRE, width=85
            )
            self.statsListCtrl.InsertColumn(7, "Skew", wx.LIST_FORMAT_CENTRE, width=85)
            self.statsListCtrl.InsertColumn(
                8, "Kurtosis", wx.LIST_FORMAT_CENTRE, width=85
            )
        ...
        try:
            for j, row in enumerate(self.normdata):
                ...
                (
                    ...
                    skew,
                    kurtosis,
                ) = tnseq_tools.get_data_stats(row)
                self.statsListCtrl.InsertStringItem(self.index_stats, name)
                self.statsListCtrl.SetStringItem(
                    self.index_stats, 1, "%1.1f" % (density * 100.0)
                )
                self.statsListCtrl.SetStringItem(
                    self.index_stats, 2, "%1.1f" % (meanrd)
                )
                self.statsListCtrl.SetStringItem(
                    self.index_stats, 3, "%1.1f" % (nzmeanrd)
                )
                self.statsListCtrl.SetStringItem(
                    self.index_stats, 4, "%1.1f" % (nzmedianrd)
                )
                ...
                self.statsListCtrl.SetStringItem(
                    self.index_stats, 8, "%1.1f" % (kurtosis)
                )
                print(
                    self.qc_prefix, "Adding dataset (%d): %s" % (self.index_stats, name)
                )
                self.index_stats += 1
        except Exception as e:
            print(self.qc_prefix, "Error:", e)
        ...
        try:
            for i, reads in enumerate(self.normdata):
                ...
                n, bins, patches = ax.hist(
                    truncnzreads, density=1, facecolor="c", alpha=0.75, bins=100
                )
                plt.xlabel("Reads")
                ...
                plt.clf()
                ax = fig.add_subplot(111, frame_on=False)
                ((qtheoretical, qdata), (slope, intercept, r)) = scipy.stats.probplot(
                    truncnzreads, dist="geom", sparams=(1.0 / numpy.mean(truncnzreads),)
                )
                ...
                self.plots_list.append(
                    [fig, hist_wx_bitmap, qq_wx_bitmap, sorted_wx_bitmap]
                )
    ...
    def onStatsItemSelect(self, event):
        ...
        print(
            self.qc_prefix,
            "Showing plots for ",
            self.statsListCtrl.GetItem(ii, 0).GetText(),
        )
        self.refreshPlots()
